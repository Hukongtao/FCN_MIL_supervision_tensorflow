...
def read_tfrecord_and_decode_into_image_annotation_pair_tensors(tfrecord_filenames_queue):
    ...
    # serialized_example = Tensor("ReaderReadV2:1", shape=(), dtype=string)
    ...
    # image = Tensor("DecodeRaw:0", shape=(?,), dtype=uint8) 
    # annotation = Tensor("DecodeRaw_1:0", shape=(?,), dtype=uint8) 
    ...
    # image reshaped = Tensor("Reshape:0", shape=(?, ?, 3), dtype=uint8) 
    # annotation reshaped = Tensor("Reshape_1:0", shape=(?, ?, 1), dtype=uint8) 
