...
def get_basic_pascal_image_annotation_filename_pairs(pascal_root):

    """Returns image/annotation filenames pairs train/val splits from Pascal VOC.
    
    Returns two arrays with train and validation split respectively, that has
    image full filename/ annotation full filename pairs that were derived
    from PASCAL dataset. 
    
    The PASCAL VOC dataset can be downloaded from here:
    http://host.robots.ox.ac.uk/pascal/VOC/voc2012/VOCtrainval_11-May-2012.tar
    
    Consider specifying root full names arguments for this function after extracting them.

    
        Let T, V, X be the PASCAL segmentation challenge training, validation, and
        test sets. 
    ...
    train_image_annotation_filename_pairs = \
    get_pascal_selected_image_annotation_filenames_pairs(pascal_root,
                                                         list(pascal_train_name_set))

    val_image_annotation_filename_pairs = \
    get_pascal_selected_image_annotation_filenames_pairs(pascal_root,
                                                         pascal_val_name_set)

    return train_image_annotation_filename_pairs, val_image_annotation_filename_pairs
