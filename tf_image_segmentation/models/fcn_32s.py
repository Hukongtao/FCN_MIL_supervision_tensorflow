...
def FCN_32s(image_batch_tensor,
    ...
    with tf.variable_scope("fcn_32s") as fcn_32s_scope:
        ...
        # logits = Tensor("fcn_32s/vgg_16/fc8/BiasAdd:0", shape=(1, 12, 12, 21), dtype=float32) 
        # end_points = output from all individual layers 
        ...
        # print 'fcn_32s - end_points = ', end_points, '\n'

        # downsampled_logits_shape =  Tensor("fcn_32s/Shape:0", shape=(4,), dtype=int32)
        downsampled_logits_shape = tf.shape(logits)

        # Calculate the ouput size of the upsampled tensor
        # upsampled_logits_shape = Tensor("fcn_32s/stack:0", shape=(4,), dtype=int32) 
        ...
        # upsampled_logits =  Tensor("fcn_32s/conv2d_transpose:0", shape=(?, ?, ?, ?), dtype=float32)
    ...
    return upsampled_logits, vgg_16_variables_mapping, logits


    # logits should be returned for weak supervision??? 